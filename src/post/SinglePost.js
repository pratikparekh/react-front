...
class SinglePost extends Component {
	...
	componentDidMount = () =>{
		...
		singlePost(postId).then(data=>{
			if(data.error){
				...
				const posterId = this.state.post.postedBy ? `/user/${this.state.post.postedBy._id}` : "";
				const posterName = this.state.post.postedBy ? this.state.post.postedBy.name : " Unknown ";
	...
	renderPost = (post) =>{
		...
		const posterIdOnly = post.postedBy ? `${post.postedBy._id}` : "";
		...
		return( 
					  ...
					  <img src={`${process.env.REACT_APP_API_URL}/post/photo/${post._id}`}  alt={post.title}  onError={i=>i.target.src=`${DefaultPost}`} className="img-thumbnail mb-3" style={{height:"300px",width:"100%",objectFit:"cover"}}/>
					    ...
					    <div className="d-inline-block mt-5">
							...
							<Link to={`/`}className="btn btn-raised btn-primary btn-sm mr-5">Back to all Posts</Link>


							{isAuthenticated().user && isAuthenticated().user._id === posterIdOnly &&

								<>

							
							<Link to={`/post/edit/${post._id}`} className="btn btn-raised btn-warning btn-sm mr-5">Edit Post</Link>
	...
	render() {
		...
		const {redirectToHome,redirectToSignin,comments} =this.state
